...
var spreadsheetID = "1xxE-TAa-n8w6EuzN6oaMB2cJOOSjdS1SzZTlqRIDv-g";
...
$(document).ready(function () {
  ...
  if (remote) {
    ...
    /* -------- Local Excel (rondo.xlsx) via XLSX --------
       Requirements:
       <script src="https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js"></script>
       Place rondo.xlsx next to this page with sheets: Site, Pages, Items
    ----------------------------------------------------- */
    (async () => {
      try {
        ...
        if (!siteWS || !pagesWS || !itemsWS) throw new Error('Workbook must have sheets named: Site, Pages, Items');
        ...
        console.error('Local XLSX load failed:', err);
        $('main').attr('aria-busy', 'false').append(
          '<p style="color:#c00">Failed to load <code>rondo.xlsx</code>: ' + String(err) + '</p>'
        );
      }
    })();

    // (Keeping your earlier localizing note, optional)
    // $('.localizing').html('<h5>Localizing Tools</h5><p>This Rondo site is currently running on local files.');
  }
});
...
function itemsDataTable(itemsJsonData) {
  ...
  itemsTable = $('#items').DataTable({
    ...
    // (Removed per-draw click-binding; we'll delegate once in initComplete)
    initComplete: function(){
      var api = this.api();

      // Delegate row clicks once (survives all redraws)
      if (!$('#items').data('cardClickBound')) {
        $('#items tbody').on('click', 'tr', function () {
          const rowData = api.row(this).data();
          if (rowData) modalBuild(rowData);
        });
        $('#items').data('cardClickBound', true);
      }
      ...
      // Toggle SearchBuilder (hidden by default)
